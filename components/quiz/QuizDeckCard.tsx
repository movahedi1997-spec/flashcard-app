...
export default function QuizDeckCard({ deck, onOpen, onRename, onDelete, onStudy }: Props) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const palette = PALETTES[deck.color] ?? PALETTES.indigo!;
  ...
  return (
    <>
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <span className="inline-block text-white/80 text-[10px] font-bold uppercase tracking-widest bg-white/20 px-2 py-0.5 rounded-full mb-1.5">
                Quiz · {deck.questionCount} {deck.questionCount === 1 ? 'question' : 'questions'}
              </span>
              <h3
            ...
            <div className="relative shrink-0">
              ...
              {menuOpen && (
                <div
                  className="absolute end-0 top-full mt-1 w-40 bg-white rounded-xl shadow-lg border border-slate-200 py-1 z-50"
      ...
      <ConfirmDialog
        ...
        message={`Delete "${deck.title}" and all its questions? This cannot be undone.`}
      />
    </>
  );
}
