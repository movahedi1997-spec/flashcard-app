...
export default function SignupPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (form.password.length < 8) { setError(t('errorPassword')); return; }
    if (!dob) { setError(t('errorDob')); return; }
    if (calculateAge(dob) < 13) { setError(t('errorAge')); return; }
    setLoading(true);
    setError('');
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="name" className="mb-1.5 block text-sm font-medium text-gray-700">{t('nameLabel')}</label>
            ...
            <div>
              <label htmlFor="email" className="mb-1.5 block text-sm font-medium text-gray-700">{t('emailLabel')}</label>
            ...
            <div>
              <label htmlFor="password" className="mb-1.5 block text-sm font-medium text-gray-700">
                {t('passwordLabel')}{' '}<span className="text-gray-400">{t('passwordHint')}</span>
              </label>
              <div className="relative">
                <input id="password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" required
                  value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                  placeholder="••••••••"
                  className="w-full rounded-xl border border-gray-200 px-4 py-3 pe-11 text-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
                />
            ...
            <div>
              <label htmlFor="dob" className="mb-1.5 block text-sm font-medium text-gray-700">
                {t('dobLabel')}{' '}<span className="text-gray-400">{t('dobHint')}</span>
              </label>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          {t('hasAccount')}{' '}
          <Link href="/login" className="font-semibold text-indigo-600 hover:underline">{t('loginLink')}</Link>
        </p>
      </div>
    </div>
  );
